...
Les formules mat{\'e}matiques sont plac{\'e}es :
	\begin{itemize}
		\item entre \$ et \$.
		\item entre \textbackslash( et \textbackslash)
		\item \textbackslash begin\{math\} et \textbackslash end\{math\}
	\end{itemize}
 
 ou bien séparée du reste :
	\begin{itemize}
		\item entre \$\$ et \$\$.
		\item entre \textbackslash [ et \textbackslash ]
		\item \textbackslash begin\{displaymath\} et\textbackslash end\{displaymath\}
	\end{itemize}
...
\frame[containsverbatim]{
	...
	\begin{LTXexample}
		Une formule ($a^2 + b^2 = c^2$) est pr{\'e}sente dans cette ligne.
		Cette formule ($$a^2 + b^2 = c^2$$) est {\`a} l'{\'e}cart.
	\end{LTXexample}
}

\frame[containsverbatim]{
	\frametitle{Puissances et indices}
	Les puissances s'utilisent avec \^ et les indices avec \_ :
...
\frame[containsverbatim]{
	\frametitle{Les fractions}
	Pour les fractions, on utilise la commande frac.
	
	Si l'on a besoin de grandes parenthèses, on peut utiliser \textbackslash left\(   et \textbackslash right\)
	
	\begin{LTXexample}
	$\frac{n!}{k!(n-k)!} = \binom{n}{k}$
	
	$\left(a + \frac{1}{a}\right)\left(b + \frac{1}{b}\right) = ab + \frac{2}{ab} - 2$
	\end{LTXexample}
}
...
En savoir plus : \cite{tikz,Inkscape}
}

\frame[containsverbatim]{
	\frametitle{Tikz}
	TikZ permet aussi des tracer des fonctions.	
	\begin{LTXexample}
		\begin{tikzpicture}
			\begin{axis}[width=190pt,height=150pt,
				xlabel=$x$, 
				ylabel={$f(x) = x^2 - x +4$}] 
			\addplot {x^2 - x +4}; 
			\end{axis}
...
\frame[containsverbatim]{
	\frametitle{Gestion des sources}
	
	Si vous travaillez sur un document \LaTeX conséquent, seul ou à plusieurs, vous souhaiterez sans doute garder un historique de vos modifications. Pour celà, il existe plusieurs méthodes :
	
	\begin{itemize}
		\item latexdiff\cite{latexdiff}, qui permet de comparer 2 versions d'une source
		\item un contrôleur de code source, Git\cite{Git, BitBucket} par exemple
	\end{itemize}
}
...
\frame[containsverbatim]{
	...
	\begin{thebibliography}{99}
		...
		\bibitem{tikz} http://math.et.info.free.fr/TikZ/bdd/TikZ-Impatient.pdf
		...
		\bibitem{latexdiff} https://fr.sharelatex.com/blog/2013/02/16/using-latexdiff-for-marking-changes-to-tex-documents.html
		\bibitem{Git} https://windows.github.com/
		\bibitem{BitBucket} https://bitbucket.org/
		\bibitem{Inkscape}
		https://inkscape.org/fr/
		http://www.cpt.univ-mrs.fr/~masson/latex/Cours-LaTeX-A4-07.pdf	
	\end{thebibliography}
}
...
\frame[containsverbatim]{
	\frametitle{Template}
	Vous trouverez ici un modèle vide de document \LaTeX qui servira de base pour les exercices qui vont suivre : \newline
	
	\begin{center}
		\url{http://tiny.cc/LatexMinimal}
	\end{center}
	
	--
	
	L'url complète :
	\url{https://raw.githubusercontent.com/MC-Course/LaTeX/master/Cours%201/Templates/Minimal.tex}
